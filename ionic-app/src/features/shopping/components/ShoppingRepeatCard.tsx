...
import './ShoppingRepeatCard.css';

interface ShoppingRepeatCardProps {
    value?: Recurrence;
    dueDate?: number;
...
const ShoppingRepeatCard: React.FC<ShoppingRepeatCardProps> = ({ value, dueDate, onChange }) => {
    ...
    if (!isActive && !expanded) {
        return (
            <div
                className="shop-repeat-card shop-repeat-card--empty"
            ...
            >
                <IonIcon icon={repeatOutline} className="shop-repeat-card-icon" />
                <div className="shop-repeat-card-body">
                    <span className="shop-repeat-card-label">Repeat</span>
                    <span className="shop-repeat-card-action">Set a schedule</span>
                </div>
                <IonIcon icon={chevronDownOutline} className="shop-repeat-card-chevron" />
            </div>
        );
    }
    ...
    if (isActive && !expanded) {
        return (
            <div
                className="shop-repeat-card shop-repeat-card--active"
                onClick={() => setExpanded(true)}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
            ...
            >
                <IonIcon icon={repeatOutline} className="shop-repeat-card-icon" />
                <div className="shop-repeat-card-body">
                    <span className="shop-repeat-card-label">Repeats: {summary}</span>
                    {nextOccurrence && (
                        <span className="shop-repeat-card-action">Next: {formatDateShort(nextOccurrence.getTime())}</span>
                    )}
                </div>
                <IonIcon icon={chevronDownOutline} className={`shop-repeat-card-chevron ${expanded ? 'is-open' : ''}`} />
            </div>
        );
    }
    ...
    /* ── Expanded editor ── */
    return (
        <div className="shop-repeat-card shop-repeat-card--expanded">
            <div
                className="shop-repeat-card-head"
            ...
            >
                <IonIcon icon={repeatOutline} className="shop-repeat-card-icon" />
                <span className="shop-repeat-card-head-title">Repeat</span>
                <IonIcon icon={checkmarkOutline} className="shop-repeat-card-done" />
            </div>

            <div className="shop-repeat-freq-grid">
                {FREQUENCIES.map((f) => (
                    <button
                        key={f.value}
                        className={`shop-repeat-chip${isActiveFreq(f.value) ? ' is-active' : ''}`}
                ...
                <button
                    className={`shop-repeat-chip${customMode ? ' is-active' : ''}`}
                    onClick={handleCustomClick}
                >
                    <IonIcon icon={optionsOutline} className="shop-repeat-chip-icon" />
                    Custom
                </button>
            </div>

            {value?.frequency === 'weekly' && !customMode && (
                <div className="shop-repeat-weekday-row">
                    {WEEKDAY_LABELS.map((label, i) => (
                        <button
                            key={i}
                            className={`shop-repeat-weekday-btn${isWeekSelected(i) ? ' is-active' : ''}`}
            ...
            {customMode && (
                <div className="shop-repeat-custom-row">
                    <span className="shop-repeat-custom-label">Every</span>
                    <div className="shop-repeat-stepper">
                        <button
                            className="shop-repeat-stepper-btn"
                        ...
                        <span className="shop-repeat-stepper-value">{customInterval}</span>
                        <button
                            className="shop-repeat-stepper-btn"
                            onClick={() => handleIntervalChange(1)}
            ...
            {value && (
                <div className="shop-repeat-end">
                    <div className="shop-repeat-end-row">
                        <span className="shop-repeat-end-label">End repeat</span>
                        <IonToggle
                    ...
                    {value.endType === 'until' && (
                        <div className="shop-repeat-end-calendar">
                            <IonDatetime
                                presentation="date"
            ...
            {value && nextOccurrence && (
                <div className="shop-repeat-preview">
                    <IonIcon icon={calendarOutline} />
                    <span>Next: <strong>{formatDateShort(nextOccurrence.getTime())}</strong></span>
                </div>
            )}

            <button className="shop-repeat-remove" onClick={handleRemove}>
...
export default ShoppingRepeatCard;
